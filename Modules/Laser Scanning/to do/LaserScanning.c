...
typedef struct {
	...
	double						stimDuration;			// Time in [ms] during which a stimulation takes place at the given ROI. This time together with the stimulation delay is less than 
														// or equal to the holding time.
...
typedef enum {
	...
	PointAveraging_All									// Performs the arithmetic average of all fluorescence pixels within the galvo hold time. The sampling rate at all ROIs is 1/sequencePeriod
														// while the number of acquired data points is equal to nSequenceRepeat.
...
static PointJump_type*				init_PointJump_type									(Point_type* point, double holdTime, double stimDelay, double stimDuration, double jumpTime, BOOL record);
	...
	switch (event) {
		...
		case EVENT_COMMIT:
			...
			switch (control) {
				...
				case ROITab_HoldTime:
					
					// get point ROI index
					GetCtrlIndex(panel, ROITab_ROIs, &listItemIdx);
					// get selected point jump from list
					pointJump = *(PointJump_type**) ListGetPtrToItem(scanEngine->pointJumpSettings->pointJumps, listItemIdx+1);
					// update hold time
					GetCtrlVal(panel, control, &pointJump->holdTime);
					// round up to a multiple of galvo sampling
					pointJump->holdTime = ceil(pointJump->holdTime * 1e-3 * scanEngine->galvoSamplingRate) * 1e3/scanEngine->galvoSamplingRate;
					SetCtrlVal(panel, control, pointJump->holdTime);
				...
				case ROITab_StartDelay:
					...
					// round up to a multiple of galvo sampling
					scanEngine->pointJumpSettings->startDelay = ceil(scanEngine->pointJumpSettings->startDelay * 1e-3 * scanEngine->galvoSamplingRate) * 1e3/scanEngine->galvoSamplingRate;
					SetCtrlVal(panel, control, scanEngine->jumpStartDelay);
					break;
					
				case ROITab_ParkedAtSequenceEnd:
					...
					if (scanEngine->pointJumpSettings->parkedAtSequenceEnd) {
						// allow sequence period to be adjusted freely beyond a minimum value
						SetCtrlAttribute(panel, ROITab_SequencePeriod, ATTR_DIMMED, FALSE);
						
					} else {
						// fix sequence period to its minimum value and dim control
						SetCtrlAttribute(panel, ROITab_SequencePeriod, ATTR_DIMMED, TRUE);
					}
					break;
					
				case ROITab_ParkedAfterEachPoint:
					
					GetCtrlVal(panel, control, &scanEngine->pointJumpSettings->parkedAfterEachPoint);
		...
		case EVENT_KEYPRESS:
			...
			switch (control) {
				...
				case ROITab_ROIs:
					...
					if (!nROIs) {
						SetCtrlAttribute(panel, ROITab_HoldTime, ATTR_DIMMED, 1);
		...
		case EVENT_MARK_STATE_CHANGE:
			...
			switch (control) {
				...
				case ROITab_ROIs:
					...
					GetNumCheckedItems(panel, control, &nROIs);
					if (nROIs) {
						SetCtrlAttribute(panel, ROITab_HoldTime, ATTR_DIMMED, 0);
...
static PointJump_type* init_PointJump_type (Point_type** point, double holdTime, double stimDelay, double stimDuration, double jumpTime, BOOL record)
{
	...
	pointJump->point 			= *point;
	*point 						= NULL;
	pointJump->holdTime 		= holdTime;
	pointJump->stimDelay 		= stimDelay;
	pointJump->stimDuration 	= stimDuration;
	pointJump->jumpTime		 	= jumpTime;
	pointJump->record		 	= record;
...
static void	discard_PointJump_type (PointJump_type** pointJumpPtr)
{
	...
	discard_Point_type(&pointJump->point);
			...
			if (scanEngine->baseClass.activeDisplay == imgDisplay) {
				...
				switch (addedROI->ROIType) {
					...
					case ROI_Point:
						...
						SetCtrlAttribute(scanEngine->baseClass.ROIsPanHndl, ROITab_HoldTime, ATTR_DIMMED, 0);
		...
		switch (ROI->ROIType) {
			...
			case ROI_Point:
				...
				if (activeROIAvailable) {
					SetCtrlAttribute(scanEngine->baseClass.ROIsPanHndl, ROITab_HoldTime, ATTR_DIMMED, FALSE);
					...
					SetCtrlAttribute(scanEngine->baseClass.ROIsPanHndl, ROITab_HoldTime, ATTR_DIMMED, TRUE);
